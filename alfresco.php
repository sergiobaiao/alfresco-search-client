...
define( 'ALFRESCO_SEARCH_VERSION', '1.5.0' );

function alfresco_search_force_locale( $locale, $domain ) {
    if ( 'alfresco-search' !== $domain ) {
        return $locale;
    }

    $preferred_locale = 'pt_BR';
    $preferred_mo = plugin_dir_path( __FILE__ ) . 'languages/' . $domain . '-' . $preferred_locale . '.mo';

    if ( file_exists( $preferred_mo ) ) {
        return $preferred_locale;
    }

    return 'en_US';
}
add_filter( 'plugin_locale', 'alfresco_search_force_locale', 10, 2 );
...
/*    wp_enqueue_style(
    ...
    wp_localize_script('alfresco-search-js', 'alfrescoSearch', array(
        ...
        'loadingMessage' => __('Loading results...', 'alfresco-search'),
        'version' => ALFRESCO_SEARCH_VERSION
    ));
}
...
function alfresco_search_ajax_handler() {
    ...
    $file_type = isset($_GET['file_type']) ? sanitize_text_field(wp_unslash($_GET['file_type'])) : 'pdf';
...
function alfresco_search_shortcode($atts){
    ...
    $output .= '<form id="alfresco-search-form" method="get" action="' . esc_url( get_permalink() ) . '">';
    ...
    $output .= '<p class="mt-2 text-right text-xs text-gray-500">' . sprintf(__('Plugin version: %s', 'alfresco-search'), esc_html(ALFRESCO_SEARCH_VERSION)) . '</p>';
    $output .= '</form>';
    ...
    $output .= '<div class="alfresco-search-results" id="alfresco-search-results" aria-live="polite" aria-busy="false">';
